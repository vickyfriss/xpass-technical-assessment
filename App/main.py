...
def generate_coach_insights(player_name, player_pos, diff_pct, easy_share, df):
    ...
    if len(player_difficult) > 30:
        ...
        if diff < -0.05:
            insights.append("Underperforms on difficult passes, completing significantly fewer than expected.")
        elif diff > 0.05:
            insights.append("Performs very well on difficult passes, completing more than expected.")
        else:
            insights.append("Performs on difficult passes roughly as expected based on pass difficulty.")
    else:
        ...
        if selected_player:
            ...
            def custom_metric(label, value):
                ...
                ">
                    ...
                    <div style="font-size:{value_size}px; font-weight:600;">{value}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            if num_difficult >= MIN_DIFFICULT_PASSES:
                ...
                st.markdown(f"**Difficult Passes:** {num_difficult} attempts, {completed} completed vs {expected_completed:.2f} expected ({diff_perf:+.2f}), completion {diff_pct:.0%} vs expected {expected_pct:.0%}")
            else:
                diff_pct = 0
