...
def plot_pass_map(player_name, df, team_name="Everton", frac=0.1, seed=14):
    """
    ...
    for _, p in passes.iterrows():
        ...
        col = cmap(norm(p["xP"]))   # correct mapping now
    ...
    ax.set_title(f"{player_name} ({team_name}) - Pass Map ({frac*100:.0f}% of passes)", fontsize=16)
